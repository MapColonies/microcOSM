...
MERGE_CONF_BASE_URL_HEADER = "=file://"
DATA_FOLDER = "data"
...
def run_subprocess(command):
    run_command(command,
                ...
                (lambda: log.info('Command finished successfully')))
...
def overwrite_file(file_path, content, throw_not_found=False):
    try:
...
def log_and_raise_custom_exception(exception_message):
...
def extract_positive_number(state_file_content, key):
    try:
        value = extract_value(
            state_file_content, key, True, '=', False, 0)
        if value < 0: raise ValueError()
        return value
    except:
        log_and_raise_custom_exception(f"{key} must be a positive integer")
...
def extract_value(content, key, is_int_value=True, split_value='=', throw_not_found=False, none_found=-1):
    """
    ...
    Args:
        ...
        split_value (str): the separator between the key and it's value to be splitted by
        throw_not_found (bool): a flag determines if to raise a LookupError
        none_found (any): the value returned upon not finding the key in the content while not throwing an error
    Raises:
    ...
    """
    if is_int_value:
        found = re.search(fr'{key}=\d+', content)
    else:
        found = re.search(fr"{key}=\S+", content)
    if not found:
        if throw_not_found:
    ...
    value = found.group(0).split(split_value)[1]
    try:
        if is_int_value:
            value = int(value)
        else:
...
def parse_time_unit_config(config_content, fetch_from_file_system = False):
    ...
    """
    interval_length = extract_positive_number(config_content, 'intervalLength')
    max_interval = extract_positive_number(config_content, 'maxInterval')
    if fetch_from_file_system:
        sub_time_unit_data_dir = extract_value(config_content, 'baseUrl', False, MERGE_CONF_BASE_URL_HEADER)
    else:
        sub_time_unit_data_dir = extract_value(config_content, 'baseUrl', False, REPLICATION_URL + '/')
...
def http_fetch_starting_state(time_unit):
    ...
    if sub_time_unit_state_content == -1:
        return False
    ...
    for i in range(max_sequence_number + 1):
        top_dir = parse_integer_to_directory_number(int(i / 1000000))
        bottom_dir = parse_integer_to_directory_number(int(i / 1000))
        state_value = parse_integer_to_directory_number(int(i % 1000)) 
...
def set_state_file_content(time_unit_dir, state_content):
    '''
    Overwrites a timeunit state file in two levels, it's own replication and it's base upon it merges by.
    ...
    '''
    time_unit_state = os.path.join(time_unit_dir, STATE_FILE)
    overwrite_file(time_unit_state, state_content)
    time_unit_data_state = os.path.join(time_unit_dir, DATA_FOLDER, STATE_FILE)
    overwrite_file(time_unit_data_state, state_content)
...
def init_time_unit_merge(time_unit):
    ...
    if not os.path.isfile(time_unit_state_path):
        ...
        found = http_fetch_starting_state(time_unit)
        if not found:
            log_and_exit('could not find starting state from the based on time unit')
...
def load_merge_configuration():
    ...
    if TIME_UNIT_TO_MERGE not in Time_Unit._member_names_:
        log.error(f"time unit {TIME_UNIT_TO_MERGE} cannot be merged.")
        raise ValueError()
...
def get_current_state():
    ...
    return { 'sub_sequence_number':
            extract_positive_number(time_unit_state_content, SEQUENCE_NUMBER),
                'sequence_number':
                     extract_positive_number(time_unit_data_state_content, SEQUENCE_NUMBER) }

def get_sleep_period():
    """
    Calculates the time in seconds for the next merge job to be scheduled
    """
    try:
        last_state_path = os.path.join(time_unit.get_path(), DATA_FOLDER, STATE_FILE)
        last_state_content = read_file(last_state_path, True)
        last_merge_timestamp = extract_value(last_state_content, 'timestamp', False)
        last_merge_datetime = datetime.strptime(last_merge_timestamp, r'%Y-%m-%dT%H\:%M\:%SZ')
        next_merge_datetime = last_merge_datetime + timedelta(seconds=time_unit.interval_length)
        delta = (time_zone_to_utc(next_merge_datetime) - datetime.now(tz=timezone.utc)).total_seconds()
        if delta < 0:
            return 0
        return delta
    except:
        log_and_raise_custom_exception('could not determine sleep period for the next merge to accord')

def scheduale_next_merge():
    sleep_period = get_sleep_period()
...
def merge_job():
    ...
    while True:
        log.info(f"state before merge job: { get_current_state() }")
        log.info('running osmosis merge-replication-files command')
        run_subprocess(f"{OSMOSIS_DIR} --merge-replication-files workingDirectory={time_unit.get_path()}")
        log.info(f"state after merge job: { get_current_state() }")
        scheduale_next_merge()

def main():
    log.info('replication-merger container started')
    load_merge_configuration()
    merge_job()
